...
static void writemessage(Display *dpy, Window win, int screen)
{
  ...
  XAllocNamedColor(dpy, DefaultColormap(dpy, screen), text_color, &color, &dummy);
  ...
  for (i = j = k = 0; i <= len; i++)
  {
    ...
    if (i == len || message[i] == '\n')
    {
      ...
      XDrawString(dpy, win, gc, width + tab_size * tab_replace, height + 0 * k, message + j, i - j);
  ...
  /* xsi should not be NULL anyway if Xinerama is active, but to be afe */
...
static struct lock *lockscreen(Display *dpy, struct xrandr *rr, int screen)
{
  ...
  lock->win = XCreateWindow(dpy, lock->root, 0, 0, DisplayWidth(dpy, lock->screen), DisplayHeight(dpy, lock->screen), 0, DefaultDepth(dpy, lock->screen), CopyFromParent, DefaultVisual(dpy, lock->screen), CWOverrideRedirect | CWBackPixel, &wa);
  ...
  for (i = 0, ptgrab = kbgrab = -1; i < 6; i++)
  {
    ...
    {
      ptgrab = XGrabPointer(dpy, lock->root, False, ButtonPressMask | ButtonReleaseMask | PointerMotionMask, GrabModeAsync, GrabModeAsync, None, invisible, CurrentTime);
    }
